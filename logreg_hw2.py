...
def calculateNegativeLogLikelihood(X,y,w):
	# matmul = lambda x,y: np.matmul(y,x)
	# l = vec(logistic, 1, np.transpose(w) * X)
	# nll = -1 * vec(matmul, 1, vec(np.log, 1, l), y) + vec(matmul, vec(np.log, 1, 1 - l), 1-y)

	# z = np.apply_along_axis(np.matmul, 1, X, w)
	# l = logistic(z)
	l = logistic(X@w)
	nll = (y.T @ np.log(l + 0.0000001)) + ((1 - y).T @ np.log(1 - l + 0.0000001))
	return -nll[0][0]
